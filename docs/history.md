...
- Fixed custom-upload inference deadlocking in Keras `model.predict()` by using direct forward inference, applying softmax to the model logits, passing the full 9-class probability matrix to monotonic alignment, and scoping MediaPipe VIDEO-mode processors per source video.
- Added `scripts/preprocess_pro_videos.py` and `src/pro_preprocessing.py` to generate versioned, hash-validated, stem-preserving CSV/JSON caches for professional reference videos. Generated all 16 caches, integrated cached runtime loading with a live fallback, and reduced the measured `kin-1` end-to-end runtime from 7.63 seconds to 4.06 seconds.
- Uploaded the generated `benchmark/pro_preprocessed/` artifacts to the `sagsan/golf-swing-analyzer-dataset` Hugging Face repository and updated cold-start asset synchronization to refresh an incomplete professional cache.
- Added zero-padded source frame indices to both synchronized dashboard video labels: `USER SWING (FXXX)` and the temporally mapped professional `REFERENCE FXXX` label.
- Recalibrated the Down-The-Line spine tilt at Address rule from 30°–45° to 24°–42° based on the curated professional reference clips, and synchronized the coaching report and video scorecard labels. Logged a future enhancement for borderline tolerance bands while retaining hard warnings for excessively upright or horizontal posture.
- Removed the non-functional Streamlit `Slow-Motion Video Speed` control. Changing it only triggered a full analysis/video render and did not provide interactive playback-speed control; custom uploads now use the pipeline's fixed default render speed.
